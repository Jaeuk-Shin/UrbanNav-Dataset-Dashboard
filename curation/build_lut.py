...
def build_lut(
    ...
    if output_path is None:
        output_path = str(
            Path(db_path).parent
            / f"lut_{split}_cs{context_size}_wp{wp_length}.npz"
        )
    ...
    for row in tqdm(rows, desc=f"Building LUT ({split})"):
        ...
        usable = len(mask) - window_size
        if usable <= 0:
            continue

        total_possible += usable // interval
        ...
        for pose_start in range(0, usable, interval):
            if mask[pose_start: pose_start + window_size].all():
                lut_entries.append((seg_local_idx, pose_start))
        video_ranges.append((start_lut, len(lut_entries)))
    ...
    np.savez(
        ...
        segment_paths=np.array(segment_paths, dtype=object),
        lut=np.array(lut_entries, dtype=np.int32),
        video_ranges=np.array(video_ranges, dtype=np.int32),
        filter_cfg=cfg_json,
        pose_step=pose_step,
    ...
    conn.execute(
        ...
        (lut_name, split, context_size, wp_length, pose_step,
         cfg_json, output_path, len(lut_entries)),
    )
    conn.commit()
    conn.close()
